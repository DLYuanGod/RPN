...
def selection_mat(embeddings,prob):
    dim1,dim2,dim3 = embeddings.size()
    global selection_matrix
    selection_matrix = torch.from_numpy(np.random.choice([1., 0.], size=dim1*dim2*dim3, p=[prob,1-prob]).reshape(dim1,dim2,dim3)).cuda().to(torch.float32)
